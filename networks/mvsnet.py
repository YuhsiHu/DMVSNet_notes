...
class DepthNet(nn.Module):
    ...
    def forward(self, cost_reg, depth_values, num_depth, interval, prob_volume_init=None, stage=0):
        prob_volume = F.softmax(cost_reg, dim=2) # B G=2+2 D H W
        depth_sub_plus = depth_regression(prob_volume, depth_values=depth_values.unsqueeze(1), axis=2) # B G=2+2 H W

        depth_sup_plus_small, depth_sup_plus_huge = depth_sub_plus.split([2,2], dim=1) # (B 2 H W) (B 2 H W)
        
        small_min, small_max = depth_sup_plus_small.min(1)[0], depth_sup_plus_small.max(1)[0] # (B H W) (B H W)
        huge_min, huge_max = depth_sup_plus_huge.min(1)[0], depth_sup_plus_huge.max(1)[0] # (B H W) (B H W)
        huge_min_d, huge_max_d = 2 * huge_min - huge_max, 2 * huge_max - huge_min # (B H W) (B H W)
        small_min_d, small_max_d = 2 * small_min - small_max, 2 * small_max - small_min # (B H W) (B H W)
        ...
        coors = torch.stack( 
            ...
            axis=-1).to(depth_sub_plus[:,0].device) # B H W 2
        
        mask_00 = ((coors[:,:,:,0]%4==0)&(coors[:,:,:,1]%2==0)) # B H W
        mask_01 = ((coors[:,:,:,0]%4==0)&(coors[:,:,:,1]%2==1)) # B H W
        mask_10 = ((coors[:,:,:,0]%4==1)&(coors[:,:,:,1]%2==0)) # B H W
        mask_11 = ((coors[:,:,:,0]%4==1)&(coors[:,:,:,1]%2==1)) # B H W
        mask_20 = ((coors[:,:,:,0]%4==2)&(coors[:,:,:,1]%2==0)) # B H W
        mask_21 = ((coors[:,:,:,0]%4==2)&(coors[:,:,:,1]%2==1)) # B H W
        mask_30 = ((coors[:,:,:,0]%4==3)&(coors[:,:,:,1]%2==0)) # B H W
        mask_31 = ((coors[:,:,:,0]%4==3)&(coors[:,:,:,1]%2==1)) # B H W
        ...
        with torch.no_grad():
            ...
            photometric_confidence = 2 * (temp_photometric_confidence - 0.5) # B H W
    ...
    def refine(self, cost_reg, depth_values, num_depth, interval,alpha=5):
        prob_volume = F.softmax(cost_reg*alpha, dim=2)  # (b, 4, ndepth, h, w)
...
class CostAgg(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, stage_idx):
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            ...
            warped_volume, _ = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values) # B C D H W

            similarity=(warped_volume.view(b, c//2, 2, warped_volume.shape[2], h, w)*(ref_volume.view(b,c//2,2,1,h,w))).mean(1) # B G=2 D H W
            if self.training:
                similarity_sum = similarity_sum + similarity # B G=2 D H W
            else:
...
class MVSNet(nn.Module):
    def __init__(self, ndepths, depth_interval_ratio, cr_base_chs=None, fea_mode="fpn", agg_mode="variance", depth_mode="regression", winner_take_all_to_generate_depth=True, inverse_depth=False):
        ...
        self.cr_base_chs = cr_base_chs # [8, 8, 8]
        self.num_stage = len(ndepths)
        self.inverse_depth = inverse_depth # whether to use inverse depth
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        :return:
        """
        depth_interval = (depth_values[0, -1] - depth_values[0, 0]) / depth_values.size(1) # number
        ...
        for stage_idx in range(self.num_stage):
            ...
            stage_shape = [ori_shape[0] // int(stage_scale), ori_shape[1] // int(stage_scale)] # [H, W] for this stage
            ...
            # cost volume
            cost_volume = self.cost_aggregation(features_stage, proj_matrices_stage, depth_range_samples, stage_idx) # B G=2 D H W
            # regularization
            cost_reg = self.cost_regularization[stage_idx](cost_volume) # B G=2+2 D H W
            # depth
            ...
            depth_values_c = outputs_stage["depth_values_c"] # B G=2+2 H W

            # refine
            features_stage = [feat["stage{}_c".format(stage_idx + 1)] for feat in features]
            cost_volume_c = self.cost_aggregation(features_stage, proj_matrices_stage, depth_values_c, stage_idx) # B 2 4 H W
            cost_reg_c = self.cost_regularization_refine[stage_idx](cost_volume_c) # B 4 4 H W
            outputs_stage_refine = self.DepthNet.refine(cost_reg_c, depth_values_c, num_depth=4, interval=interval) # B H W
            # compact to a new dict
